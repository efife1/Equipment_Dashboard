/* ============================================================
   ESP32-S3 Ethernet GPIO Client — Device Type 1: Talent Pack Decoder
   ------------------------------------------------------------
   Waveshare ESP32-S3-ETH firmware for card type 1 (see
   raspberry_pi_server/device_types.py, layout "talent_pack_decoder").
   Reads 15 digital GPIO inputs (3 decoder groups x 5 channels) and
   publishes their state over MQTT to a Raspberry Pi broker. Uses DHCP
   for IP assignment. Supports multiple units on the same network — each
   gets a unique device ID derived from its Ethernet MAC address.
   ...
   only the GPIO_PINS block is specific to this card. See
