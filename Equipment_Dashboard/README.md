...
control how each device's card is drawn: a name, an accent color, and a
label for each of the 12 monitored GPIO channels — so a card can show
*"Pump Running"* / *"High Temp Fault"* instead of *"GPIO 0"* / *"GPIO 1"*.
Manage them at `http://<pi-ip>:8080/device-types`. A device with no type
assigned still works fine — its card just falls back to generic
"GPIO 0"..."GPIO 11" labels.
...
3. Fill in the equipment name, pick a **card type** from the dropdown (or
   define a new one first via the link on that page), and optionally a
   location and notes.
...
card types at `/device-types`, or pull either as JSON from `/api/registry`
and `/api/device-types`.
